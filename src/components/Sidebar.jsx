...
const Sidebar = ({ darkMode, onCreateNote, onToggleTheme }) => {
  ...
  return (
    <div className={`w-64 flex flex-col h-screen border-r ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-gray-50 border-gray-200'}`}>
      <div className="p-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Syncscribe</h1>
            <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Meet Desai</p>
          </div>
        </div>
      </div>
